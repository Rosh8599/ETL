...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            ...
            password=os.environ.get("DB_PASSWORD", "Rosh@8599"),
...
def create_dicing_charts(df):
    ...
    # --- Key Events Data ---
    key_events = {
        (2010, 'Q1'): "Economic recovery begins after the Great Recession.",
        (2010, 'Q2'): "Stock market volatility and European debt crisis.",
        (2010, 'Q3'): "Federal Reserve signals continued support.",
        (2010, 'Q4'): "Holiday retail sales rebound.",
        (2020, 'Q1'): "COVID-19 pandemic starts, sharp employment drop in March.",
        (2020, 'Q2'): "Lockdowns and historic job losses, stimulus packages.",
        (2020, 'Q3'): "Gradual reopening, partial job recovery.",
        (2020, 'Q4'): "Vaccine development, holiday spending uncertainty.",
        (2021, 'Q1'): "Vaccination rollout, economic stimulus.",
        (2021, 'Q2'): "Strong job growth, reopening accelerates.",
        (2021, 'Q3'): "Delta variant slows recovery.",
        (2021, 'Q4'): "Supply chain issues and inflation concerns.",
        (2022, 'Q1'): "Labor market tightness, wage growth.",
        (2022, 'Q2'): "Interest rate hikes begin.",
        (2022, 'Q3'): "Inflation peaks, job market remains strong.",
        (2022, 'Q4'): "Holiday hiring, tech layoffs start.",
        (2023, 'Q1'): "Tech layoffs continue, service sector growth.",
        (2023, 'Q2'): "AI boom, manufacturing rebounds.",
        (2023, 'Q3'): "Consumer spending rises.",
        (2023, 'Q4'): "Holiday sales strong, inflation moderates.",
        (2024, 'Q1'): "Election year uncertainty.",
        (2024, 'Q2'): "Interest rates stabilize.",
        (2024, 'Q3'): "Job market steady, global tensions.",
        (2024, 'Q4'): "Holiday hiring, economic outlook cautious.",
        (2025, 'Q1'): "Post-election policy changes.",
        (2025, 'Q2'): "Infrastructure investments.",
        (2025, 'Q3'): "Tech sector expansion.",
        (2025, 'Q4'): "Year-end review, steady growth.",
        # Add more events for other years/quarters as needed
    }

    # Filter and display key events for selected interval and quarter
    selected_events = []
    for year in range(year_range[0], year_range[1] + 1):
        event = key_events.get((year, quarter))
        if event:
            selected_events.append(f"<b style='color:#fff;'>{year} {quarter}:</b> <span style='color:#fff;'>{event}</span>")

    if selected_events:
        st.markdown(
            f"""
            <div style="background:#b6f5b6;padding:18px 16px 12px 16px;border-radius:10px;margin-top:16px;">
                {'<br>'.join(selected_events)}
            </div>
            """,
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            """
            <div style="background:#b6f5b6;padding:18px 16px 12px 16px;border-radius:10px;margin-top:16px;">
                <b style='color:#fff;'>No key events available for the selected interval and quarter.</b>
            </div>
            """,
            unsafe_allow_html=True
        )
